...
# Embedded file name: C:\Users\talha.celik\Desktop\Talha\myerpv2-main\toexport\apps.py
...
class ExportData:
    ...
    def __init__(self):
        if self.app_data:
            ...
            os.path.isdir(os.getcwd() + '/data') or os.mkdir(os.getcwd() + '/data')
        else:
            os.mkdir(os.getcwd() + '/data')

    def app_init(self):
        self.app_model = apps.get_model('schema', 'AppModel')
        self.last_app_id = sorted((self.app_model.objects.values_list('id')), key=(lambda tup: tup[0]))[(-1)][0]
        self.new_app_data = {}
    ...
    def table_init(self):
        ...
        self.last_table_id = sorted((self.table_model.objects.values_list('id')), key=(lambda tup: tup[0]))[(-1)][0]
        self.new_table_data = {}
    ...
    def field_init(self):
        ...
        self.last_field_id = sorted((self.field_model.objects.values_list('id')), key=(lambda tup: tup[0]))[(-1)][0]
        self.new_field_data = {}
        ...
        for config in config_list:
            ...
            if config.name == project_name:
                ...
                if config.name in folder_list:
                    ...
                    if 'apps.py' in folder_list2:
                        if 'models.py' in folder_list2:
        ...
        for field in field_list:
            if field.name == 'id':
                ...
                if table_obj:
                    ...
                    elif type(field.default) == bool:
                        value10 = str(field.default)
                    else:
                        if type(field.default) == types.FunctionType:
                            value10 = str(field.default())
                        else:
                            if type(field.default) == types.BuiltinMethodType:
                                value10 = str(field.default())
                            else:
                                value10 = ''
