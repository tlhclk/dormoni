...
def GetRepetitiveModelDetail(request):
    ...
    max_value = 0
    min_value = 0
    rt_list = RepetitiveTransactionModel.objects.filter(repetitive_id_id=(int(pk))).order_by('-transaction_id__date')
    ...
    if r_obj:
        ...
        context_data = {'payment_data':payment_data, 'inter_val':inter_val,  'min_value':min_value,  'max_value':max_value}
        return context_data
