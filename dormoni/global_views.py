...
from functions.db.Table import ObjectFunc
from functions.db.QuerySet import ModelQueryset
    ...
    def get_form(self, form_class=None):
        ...
        for field_name, form_field in form.fields.items():
            ...
            if field.field == 'DateField':
                ...
                if field_name == "transaction_id":# TODO: İstisnai bir durum globalleştirilecek
                    mq = ModelQueryset(self.request, "TransactionModel", self.action)
                    if "transaction_id" in form.initial:
                        form.fields["transaction_id"].queryset = mq.table_model.objects.filter(id=form.initial["transaction_id"])
                    else:
                        form.fields["transaction_id"].queryset = mq.get_queryset().filter(date__gte=(datetime.now()-timedelta(days=60)).strftime("%Y-%m-%d"))
            else:
...
class GlobalUpdateView(UpdateView):
    ...
    def get_form_detail(self, obj):
        ...
        if obj:
            ...
            return 'fields:[], values:[]'
        return 'fields:[%s], values:[%s]' % (', '.join(field_list) or '', ', '.join(value_list) or '')
...
class GlobalDeleteView(DeleteView):
    ...
    def get_success_url(self):
        self.create_log()
        return redirect('/global_list/' + self.kwargs['table_name']).url
